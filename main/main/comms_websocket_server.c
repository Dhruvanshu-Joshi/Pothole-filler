...
static comms_val_t comms_val = { .test = 1.0, .speed = 0, .right_turn = false, .left_turn = false, .front = false, .back = false, .val_changed = false, .left_servo = false, .right_servo = false};
        ...
        { // if the message length was greater than zero
            ...
            switch (msg[0])
            {
                ...
                case 'U':
                    ESP_LOGI(TAG, "got message length %i: %s", (int)len - 1, &(msg[1]));
                    if(strcmp(&msg[1], "true") == 0)
                        comms_val.left_servo = true;
                    else
                        comms_val.left_servo = false;
                    break;
                case 'P':
                    ESP_LOGI(TAG, "got message length %i: %s", (int)len - 1, &(msg[1]));
                    if(strcmp(&msg[1], "true") == 0)
                        comms_val.right_servo = true;
                    else
                        comms_val.right_servo = false;
                    break;
